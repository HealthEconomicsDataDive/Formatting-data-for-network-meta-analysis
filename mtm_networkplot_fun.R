mtm_networkplot_fun <- function(t1,t2,percomparison,nameoftreatments,VAR1,VAR2,graphtitle,thickness,nodetextsize,nodesize) {
...
#in the same as previous form
	...
	if(sum(na > 2))
	{
		# Are there more than 1 trials with >2 arms?
		if(sum(na > 2) > 1)
		{
			for(i in 1:(sum(na > 2))) 
			{
		...
		# Case where only one trial with >2 arms is included
			u <- rbind(u, t(combn(unlist(TT[na>2]),2)))
		}
	}
	# Only bind if there are any studies with only 2 arms
	if(sum(na==2)){
...
#plot of the network
plot(netdata,mode="circle",displaylabels=TRUE,vertex.cex=nodethickness,boxed.label=FALSE,edge.lwd=mat_treat_new)
